...
def scrape_local_ch_reviews(business_url):
    try:
        headers = {"User-Agent": "Mozilla/5.0"}
        response = requests.get(business_url, headers=headers)

        if response.status_code != 200:
            return f"Error: Unable to access Local.ch reviews."

        soup = BeautifulSoup(response.text, "html.parser")
        overall_rating = soup.find("div", class_="rating-overall").text.strip() if soup.find("div", class_="rating-overall") else "No overall rating available"
        num_reviews = soup.find("div", class_="rating-count").text.strip() if soup.find("div", class_="rating-count") else "No review count available"

        return {"overall_rating": overall_rating, "num_reviews": num_reviews}
    except Exception as e:
        return f"Error scraping Local.ch reviews: {e}"
